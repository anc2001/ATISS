...
def main(argv):
    ...
    parser.add_argument(
        "--rendering", action="store_true", help="whether or not to do rendering"
    )
    parser.add_argument(
        "--generation_batches",
        type=str,
    ...
    raw_dataset, dataset = get_dataset_raw_and_encoded(
        ...
        filter_fn=filter_function(
            config["data"], split=config["validation"].get("splits", ["val"])
        ),
    ...
    for generation_batch in range(generation_batch_start, generation_batch_end):
        ...
        output_dir = args.output_directory / f"generated_scenes_{generation_batch}"
        ...
        for scene_num in tqdm(range(args.num_scenes)):
            ...
            scene_output_dir = output_dir / f"scene_{scene_num:04d}"
